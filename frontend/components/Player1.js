...
class Player1 extends React.Component {
  ...
  elementDrag(e) {
    ...
    if(this.refs.player.offsetLeft - pos1 > 0 && this.refs.player.offsetLeft - pos1 < (this.props.boardWidth/2) - 20) {
      this.props.changeCoord('player1downX', e.clientX);
      this.props.changeCoord('player1X', this.refs.player.offsetLeft - pos1);
    }
    if(this.refs.player.offsetTop - pos2 >= 0 && this.refs.player.offsetTop - pos2 <= 360) {
      this.setState({ downY: e.clientY, y: this.refs.player.offsetTop - pos2 });
      this.props.changeCoord('player1downY', e.clientY);
      this.props.changeCoord('player1Y', this.refs.player.offsetTop - pos2);
    }
